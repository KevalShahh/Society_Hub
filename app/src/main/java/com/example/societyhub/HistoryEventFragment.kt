 ...
 * Use the [HistoryEventFragment.newInstance] factory method to
...
class HistoryEventFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        recyclerView=view.findViewById(R.id.rv_history_event)
        var fireuser= FirebaseAuth.getInstance().currentUser
        var user= fireuser?.email
        if (user != null) {
            query=FirebaseFirestore.getInstance().collection("Members").document(user).collection("received events").whereEqualTo("paid","Yes")
