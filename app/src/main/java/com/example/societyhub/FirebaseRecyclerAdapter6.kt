...
class FirebaseRecyclerAdapter6(var context: Context, options: FirestoreRecyclerOptions<EventModel>) : FirestoreRecyclerAdapter<EventModel, EventViewHolder>(options){
    ...
    override fun onBindViewHolder(holder: EventViewHolder, position: Int, model: EventModel) {
        ...
        holder.textInputLayout.setOnClickListener {
            if (holder.textView8.text=="Not Registered") {
                var intent = Intent(context, EventNotRegisteredUserSide1::class.java)
